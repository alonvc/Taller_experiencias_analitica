...
def train_and_log(config,experiment_id='99'):
    with wandb.init(
        project="MLOps-TallerExperienciasAnalitica2024I", 
...
def evaluate_and_log(experiment_id='99',config=None,):
    ...
    with wandb.init(project="MLOps-TallerExperienciasAnalitica2024I", name=f"Eval Model ExecId-{args.IdExecution} ExperimentId-{experiment_id}", job_type="eval-model", config=config) as run:
        data = run.use_artifact('mnist-preprocess:latest')
        data_dir = data.download()
